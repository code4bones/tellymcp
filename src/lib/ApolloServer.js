...
import { moleculerMiddleware } from "./moleculerMiddleware.js";
import { renderGraphiQL } from "./mixins/graphql.js";
	...
	createHandler({ path, disableHealthCheck, onHealthCheck, playgroundOptions } = {}) {
		...
		return async (req, res) => {
			...
			if (!disableHealthCheck && req.url === "/.well-known/apollo/server-health")
				return this.handleHealthCheck({ req, res, onHealthCheck });
			...
			if (playgroundOptions && req.method === "GET") {
				...
				if (prefersHTML) {
					...
					return send(req, res, 200, renderGraphiQL(middlewareOptions), "text/html");
				}
			}
