		...
		methods: {
			...
			getServiceName(service) {
				...
				if (service.version != null)
					return `${
						typeof service.version === "number" ? `v${service.version}` : service.version
					}.${service.name}`;
			...
			createActionResolver(actionName, def = {}) {
				...
				return async (root, args, context) => {
					try {
						if (useDataLoader) {
							const dataLoaderMapKey = this.getDataLoaderMapKey(actionName, staticParams, args);
						...
						if (fileUploadArg !== null && args[fileUploadArg] !== null) {
							...
							if (Array.isArray(args[fileUploadArg])) {
								return await Promise.all(
									args[fileUploadArg].map(async uploadPromise => {
										const { createReadStream, ...$fileInfo } = await uploadPromise;
			...
			async prepareGraphQLSchema() {
				...
				try {
					...
					const wsServer = new WebSocketServer({
						server: this.server,
						path: mixinOptions.serverOptions?.path || mixinOptions?.routeOptions?.path,
					});
					...
					const serverOptions = _.omit(mixinOptions.serverOptions, ["subscriptions", "plugins"]);
					...
					const queryLogger = {
						...
						async requestDidStart(ctx) {
							...
							const { query, variables, operationName } = ctx.request;
							if (serverOptions.tracing) {
								...
								if (!_.isEmpty(variables)) {
									...
									logger.warn(`GraphQL "${operationName}"\n${cleanQuery.join("\n")}`);
								}
							}
							return {
								async didEncounterErrors(ctx) {
									const { query, variables, operationName } = ctx.request;
									const err = ctx.errors.map(e => e.message).join("\n");
									if (_.isEmpty(variables)) logger.error(`${operationName}\n${query}\n${err}`);
									else {
										logger.error(
			...
			buildLoaderOptionMap(services) {
				...
				services.forEach(service => {
					Object.values(service.actions).forEach(action => {
						...
						if (
							graphqlDefinition &&
							(graphqlDefinition.dataLoaderOptions || graphqlDefinition.dataLoaderBatchParam)
						) {
							const serviceName = this.getServiceName(service);
							const fullActionName = this.getResolverActionName(serviceName, actionName);

							if (graphqlDefinition.dataLoaderOptions) {
								this.dataLoaderOptions.set(fullActionName, graphqlDefinition.dataLoaderOptions);
							}
		...
		created() {
			...
			const route = _.defaultsDeep(mixinOptions.routeOptions, {
				aliases: {
					...
					"GET /.well-known/apollo/server-health": async function (req, res) {
						try {
							await this.prepareGraphQLSchema();
						} catch (err) {
							res.statusCode = 503;
